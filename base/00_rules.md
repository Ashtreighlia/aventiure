...
When someone wants to do something, they and the game master (GM) roll their d10 and d10.0 dice (for a total of 1-100) to decide whether the player was successful. For example, if you want to open an LVL 1 lock with an LVL 1 lock picking skill, everyone rolls once. However, if higher levels are involved, everyone rolls as many times as they have levels in the skill. Here, the highest single roll of both parties counts. If the player wins, he manages to do whatever he wanted to do. However, if the GM wins, the player screws up his action. If there is a tie on the highest roll, the player wins.
...
A player can find and use tools to get an edge doing stuff. They increase the likely hood of them succeeding. For more information look up [armour, wands, weapons and tools of prototype](01_armour_wands_weapons_tools.md).
...
The order of players in combat is determent by a d10 throw. If the GM and a player happen to roll the same value, the player goes first. If multiple players rolled the same value, the players decide their order.

### actions

Every player has 2 actions and 1 movement action in combat. They can use their actions however they want, but they cannot do the same action and skill level twice per turn. Attacking, casting a spell, using a skill or even using their normal actions as a additional movement action counts towards their used actions that turn. However, you can only use a particular skill level once per round, which means that any subsequent actions must be made with the next lower skill level. This also applies to other skills, attack skills or spells skills within the turn.

### movement

Every player can move 2 squares per used action of movement. As a measurement for distance you may choose any measurement system that fits your bill (should be around 1 meters, 3 feet or 60 barleycorn for one square).

### attack

Everyone can **attack a thing: 1**. Just like with the normal skills, you can attack anything you want, however you want, but don't expect to impress a dragon by hitting it with a rock. Here, as with a skill check, the attacker rolls their  d10 and d10.0 (for a total of 1-100) for their attack and the defender rolls with their defence skill against the attackers roll. If the attacker wins, the target takes the damage described by their weapon. Otherwise the attacker has missed their target. As with the skills before, you increase your skill level by rolling doubles.
...
By using better weapons the attacker can add modifiers to their attack roll. Same goes for the defender, if he is wearing better armour he has an advantage over the attacker. For more information look up [armour, wands, weapons and tools of prototype](01_armour_wands_weapons_tools.md).
...
As in the previous chapters, spells also have skill levels. Each player knows a number of spells based on their class and race. They can cast all of these as level 1 spells. Every player has a certain amount of spell slots according to their picked class and race.

There are three different types of spells - attack, support and utility. Utility and support spells are cast against the worst d10 and d10.0 (for 1-100) roll of the GM with the same amount of dice as the player has skill levels. Attack spells, on the other hand, are always rolled against the defence level of the attacked, the highest single die counts. In the event of a tie, the attacked wins. The damage is determined by the properties of the spell and the wand used.

Unlike spells, rituals are not directly magical influences emanating from a being, but are rather created through concentration. Therefore, rituals do not require checks, but they can only be prepared during a rest.
...
Every character starts out with a base health of 100hp. Health can be replenished by healing as a skill or as a spell. A rest does heal you to the max as well as you consume a ration while doing a rest.
...
Healing can be achieved in three ways. Either by casting a heal spell, using a potion of healing or aiding your wounds with an first aid kit. 

### level up

Levelling is mostly determined by the GM. As a rule of thumb you level up every 6 skills you learned as a character. A level up grants you full health and spell slots and can be regarded as a rest with consumed ration.

Additionally you gain 1d10 HP on your maximum health, one more spell slot. Every 2 level ups you learn one new spell.
...
Dying is always such a thing. No one wants to, but someone has to. In prototype, death is handled relatively simply. If you are at 0HP, you are unconscious. Either you manage to stay alive until someone revives you or you are done.
In prototype you only die if an enemy attacks and hits you while you are unconscious, the damage doesn't matter. So technically your teammates can keep you unconscious after a team fight, so you better be useful to them.

You can revive team mates by healing them.

---

## monsters

Since prototype is a very open game system, the monsters you use in your session are as freely inventable as the rest. So go nuts! In the following, we will roughly go into the values such as health and actions per battle as well as attacks and defence of monsters.
